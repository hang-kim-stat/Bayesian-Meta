...
Each folder contains the output of two simulation settings in Section 4. For each simulation setting and each method, two subdirectories are provided. 
  - `RData` directory
      - `rep_x.RData` contains posterior draws of **μ** and for the x-th simulation data (out of 300)
  - `DiagnosticPlot` directory
      -  The diagnostic plot of **μ** = E(**θ**<sub>l</sub>) and **Σ** = V(**θ**<sub>l</sub>) will be stored here (while running MCMC).
  - Two .csv files for each simulation setting were generated by running `W_Summary.R` in `code/[Simulation_X]/[MethodName]` folder.
