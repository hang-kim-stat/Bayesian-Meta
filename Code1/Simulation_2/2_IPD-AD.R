...
{  
  ...
  alpha = as.vector(alpha) ; w.DRM = exp(x[,1:2]%*%alpha) # for the density ratio model
  w = w.DRM * w # for the density ratio model
...
for (i_iter in 1:n_iter) {
  ...
  ##################################
  # Update theta_mat_AD, beta, and alpha 
  ##################################
  ...
  for (k in 1:K){
    ...
    if ( temp.optim$convergence==0 ){
      ...
      logNum = dmvnorm(beta_tilde_mat[k,2:3], mean=beta_vec_q[2:3], sigma=V_tilde_cube[k,2:3,2:3], log=TRUE)
      logNum = logNum + dmvnorm(bar_g_l_q, mean=rep(0,2), sigma=Sigma_g_l_q, log=TRUE) # density ratio
  ...
  for (k in 1:K){
    ...
    g_l1_vec = exp( tilde_D_x[[k]][,1:2]%*%alpha_mat[k,] ) # common for q and current
